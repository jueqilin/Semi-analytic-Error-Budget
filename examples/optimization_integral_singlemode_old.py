...
def initial(param_dir = 'params_mod4_integral.yaml'):
    # 1. Load parameters
    ...
    if controller_type is not None:
        if controller_type == 1:  # integral controller
            ...
            if gain_value is not None:
                ...
                ctrl_num_array = None
                ctrl_den_array = None
        elif controller_type == 2: # standard controller
            ctrl_order = param['optimization'].get('order')
            if ctrl_order is not None:
                gain_array = None
                n_num_standard = ctrl_order[0]+1
                n_den_standard = ctrl_order[1]+1
                ctrl_num_array = np.zeros(n_num_standard, dtype=float)
                ctrl_num_array[0] = 1.0
                ctrl_den_array = np.zeros(n_den_standard , dtype=float)
                ctrl_den_array[0] = 1.0  
            else:
                raise ValueError("Provide controller's order (standard controller)")
        else:
    ...
    x0 = np.array([gain_array[mode_index]])
    ...
    res_opti_dual_annealing = dual_annealing(
        opti_cost_func, 
        opti_bounds,
        x0=x0,
    ...
    # compare psd
    # although this case optimize the controller for single mode,
    # the build_transfer_function require the whole gain array
    gain_array_optimized = np.ones_like(gain_array) * res_opti_dual_annealing.x[0]
